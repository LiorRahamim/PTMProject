...
public class GraphSolver implements Solver<GraphPack, ArrayList<Direction>> {
	...
	public ArrayList<Direction> solve(GraphPack p) {
		int numRows = p.getMatrix().length;
		int numCols = p.getMatrix()[0].length;
			...
			for(int col = 0 ; col < numCols; col++) {
				...
				if(col > 0) {
					edges.add(new Edge(vertices.get(row*numCols+col),
							vertices.get(row*numCols+col-1),p.getMatrix()[row][col-1]));
				}
				...
				if(row > 0) {
					edges.add(new Edge(vertices.get(row*numCols+col),
							vertices.get((row-1)*numCols+col),p.getMatrix()[row-1][col]));
				}
		...
		Vertix st = vertices.get(p.getStart().getRow() * numCols + p.getStart().getCol());
		Vertix end = vertices.get(p.getFinish().getRow() * numCols + p.getFinish().getCol());
